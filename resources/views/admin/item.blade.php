            ...
            <table class="table table-striped table-hover table-bordered align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <img width="50px" height="50px" src="{{ asset('storage/items/'.$item->image) }}"
                                alt="gambar">
                        </td>
                                ...
                                <form action="{{ route('editItem', $item->id) }}" method="POST"
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-control" id="category_id{{ $item->id }}"
                                                name="category_id">
                                                <option value="">Pilih Kategori</option>
                                                @foreach($categories as $category)
                                                <option value="{{ $category->id }}" {{ $item->category_id ===
                                                    $category->id
                                                    ? 'selected' : '' }}>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <textarea class="form-control" id="description{{ $item->id }}"
                                                name="description" rows="3">{{ $item->description }}</textarea>
                                        </div>
                                        <div class="mb-3">
                                            <label for="unit_price{{ $item->id }}" class="form-label">Harga</label>
                                            <input type="number" class="form-control" id="unit_price{{ $item->id }}"
                                                name="unit_price" value="{{ $item->unit_price }}" required>
                                        </div>
                                        <div class="mb-3">
                                            <label for="image{{ $item->id }}" class="form-label">Gambar</label>
                                            <input type="file" class="form-control" id="image{{ $item->id }}"
                                                name="image">
                                        </div>
                                    </div>
